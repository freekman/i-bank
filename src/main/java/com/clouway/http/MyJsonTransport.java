...
import com.clouway.core.HibernateValidationException;
import com.clouway.core.ObjectCastException;
...
public class MyJsonTransport implements Transport {
  private HttpServletResponse response;

  @Inject
  public MyJsonTransport(HttpServletResponse response) {
  ...
  public <T> T in(InputStream in, Class<T> type) throws IOException {
    ...
    T object = getGetObjectFromJson(type, json, null);
    if (null != object) {
  ...
  private <T> T getGetObjectFromJson(Class<T> type, String json, T object) {
    Gson gson = new Gson();
    try {
